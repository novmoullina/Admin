...
class activity_4 : AppCompatActivity(), View.OnClickListener {
    ...
    private lateinit var anggotaList : MutableList<variabel_R_wisata>

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.input_wisata)
        ...
        ref.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    for (a in snapshot.children){
                        val anggota = a.getValue(variabel_R_wisata::class.java)
                        if (anggota !=null){
                            anggotaList.add(anggota)
                        }
                    }

                    val adapter = WisataAdapter( this@activity_4,
                            R.layout.activity_item_wisata, anggotaList)
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@activity_4, "error: "+error, Toast.LENGTH_LONG).show()
            }
        })

        listData.setOnItemClickListener { parent, view, position, id ->
            val anggota = anggotaList.get(position)
            val intent = Intent(this@activity_4, TambahWisata  ::class.java)
            intent.putExtra(TambahWisata.EXTRA_ID, anggota.id)
            intent.putExtra(TambahWisata.EXTRA_NAMA, anggota.nama)
    ...
    private fun simpanData(){
        ...
        val anggota = variabel_R_wisata(anggotaId!!, nama, alamat, deskripsi)
